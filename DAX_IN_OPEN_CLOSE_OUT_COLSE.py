...
"""
maxLen = len(dataset_train)-1
X_train_close = []
X_train_open = []
y_train_close = []
for i in range(60, maxLen):
    X_train_close.append(training_set_close_scaled[i-60:i, 0])
    X_train_open.append(training_set_open_scaled[i-59:i+1,0])
    y_train_close.append(training_set_close_scaled[i, 0])
X_train_close, X_train_open, y_train_close = np.array(X_train_close), np.array(X_train_open), np.array(y_train_close)
...
"""
X_train_close = np.reshape([X_train_close,X_train_open], (X_train_close.shape[0], X_train_close.shape[1], 2))
...
regressor_close.add(LSTM(units = 70, return_sequences = True, input_shape = (X_train_close.shape[1], 2)))
regressor_close.add(Dropout(0.2))

# Adding a second LSTM layer and some Dropout regularisation
regressor_close.add(LSTM(units = 70, return_sequences = True))
regressor_close.add(Dropout(0.2))
...
regressor_close.add(LSTM(units = 70))
regressor_close.add(Dropout(0.2))
...
dataset_total_open = pd.concat((dataset_train['Open'], dataset_predict['Open']), axis = 0)
"""
...
"""
inputs_close = dataset_total_close[len(dataset_total_close) - len(dataset_predict) - 60:].values
inputs_close = inputs_close.reshape(-1,1)
inputs_close = sc.transform(inputs_close)

inputs_open = dataset_total_open[len(dataset_total_open) - len(dataset_predict) - 60:].values
...
maxLen = len(inputs_close)
X_predict_close = []
X_predict_open = []
for i in range(60, maxLen):
    X_predict_close.append(inputs_close[i-60:i, 0])
    X_predict_open.append(inputs_open[i-59:i+1, 0])
X_predict_close = np.array(X_predict_close)
X_predict_open = np.array(X_predict_open)
X_predict_close = np.reshape([X_predict_close, X_predict_open], (X_predict_close.shape[0], X_predict_close.shape[1], 2))
